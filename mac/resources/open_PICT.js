define(['mac/roman', 'mac/fixedPoint'], function(macintoshRoman, fixedPoint) {
  ...
  return function(item) {
    return item.getBytes().then(function(bytes) {
      var pictDV = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
      ...
      item.with2DContext(right - left, bottom - top, function(ctx) {
        ...
        if (bytes[10] === 0x11 && bytes[11] === 0x01) {
          ...
          function region() {
            ...
            if (len > 10) {
              region.extra = bytes.subarray(pictPos + 10, pictPos + len);
            }
            pictPos += len;
            return region;
          }
          ...
          pictV1loop:
          for (var pictPos = 12; bytes[pictPos] !== 0xff; ) {
            switch(bytes[pictPos++]) {
              ...
              case 0x28: // long text
                ...
                pictPos += 4;
                var text = macintoshRoman(bytes, pictPos+1, bytes[pictPos]);
              ...
              case 0x98: // copy packed bits to clipped rect
                ...
                console.log(bytes.subarray(pictPos, pictPos + 2 + 44));
                pictPos += 2 + 44;
                console.log(pixmap);
                break pictV1loop;
              ...
              case 0xA1: // long comment
                ...
                var commentData = bytes.subarray(pictPos + 4, pictPos + 4 + len);
              ...
              default:
                console.error('unhandled PICTv1 opcode: 0x' + bytes[pictPos - 1].toString(16));
                break pictV1loop;
            }
          }
        }
        else if (bytes[10] === 0x00 && bytes[11] === 0x11
            && bytes[12] === 0x02 && bytes[13] === 0xff) {
          // version 2
